...
function ProceduralBrushPreview() {
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const { syncUniforms } = useWebGLBrushTip(canvasRef, { primary: false })
  const proc = useBrushTipStore((s) => s.procedural)
  const shape = useBrushTipStore((s) => s.shape)

  useEffect(() => {
    syncUniforms()
  }, [proc, shape, syncUniforms])

  return (
    <canvas
      ref={canvasRef}
      className="w-32 h-32 bg-[#1A1A24] rounded-xl border border-white/10 shrink-0"
      width={128}
      height={128}
    />
  )
